...
class LoRaParser(TextParser):
    def __init__(self, service: bool = False):
        ...
        self.raw_mode = self._read_config()
    
    def _read_config(self):
        """Читаем настройки из конфигурационного файла"""
        config_path = "/home/orangepi/lorasdr/lora.config"
        
        # Значение по умолчанию, если файла нет
        default_mode = False  # по умолчанию парсим
        ...
        try:
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    for line in f:
                        line = line.strip()
                        if line.startswith('lora_parser ='):
                            # Извлекаем значение после '='
                            value = line.split('=', 1)[1].strip().lower()
                            if value == 'true':
                                return False  # парсим
                            elif value == 'false':
                                return True   # прямой поток
            else:
                print(f"Config file {config_path} not found, using default mode: parsing")
                
        except Exception as e:
            print(f"Error reading config file: {e}, using default mode: parsing")
    ...
    def parse(self, msg: bytes):
        ...
        if self.raw_mode:
            # Режим напрямую - весь текст как есть
            return f"{text} {current_time}\n"
        else:
            # Режим парсинга - только сообщения после "rx msg: "
            import re
            main_msg_pattern = r'rx msg: (.+)'
            main_match = re.search(main_msg_pattern, text)
            
            if main_match:
                main_message = main_match.group(1)
                return f"{main_message} {current_time}\n"
            else:
                if "CRC invalid" in text:
                    return f"message error. {current_time}\n"
                return None
