...
#class Rtl4332Module(ExecModule):
#    def __init__(self, sampleRate: int = 250000, jsonOutput: bool = False):
#        cmd = [
#            "rtl_433", "-r", "cf32:-", "-s", str(sampleRate),
#            "-M", "time:unix" if jsonOutput else "time:utc",
...
class LoRaModule(ExecModule):
    def __init__(self, sampleRate: int = 250000, sf: int = 7, bw: int = 125000, jsonOutput: bool = False):
        cmd = [
            "/home/orangepi/miniconda3/envs/gnuradio-3.10/bin/python3",  # полный путь
            "/home/orangepi/lorasdr/lora_rx_stdin.py",
            "--samp-rate", str(sampleRate),
            "--sf", str(sf),
            "--bw", str(bw)
        ]
        super().__init__(Format.COMPLEX_FLOAT, Format.CHAR, cmd)
